...
const getCredStorageKey = (address?: string) =>
  address
    ? `trustid_local_credentials_${address.toLowerCase()}`
    : "trustid_local_credentials";
...
export function useCredentials(address?: string) {
  ...
  const fetchedCredentials: Credential[] = validIds
    .map((id, index) => {
  ...
  const credentials =
    fetchedCredentials.length > 0 ? fetchedCredentials : localCreds;
