...
const getStorageKey = (address?: string) =>
  `trustid_activity_events_${address?.toLowerCase() || "anon"}`;
const getSimulatedCredsKey = (address?: string) =>
  `trustid_simulated_creds_${address?.toLowerCase() || "anon"}`;
...
function saveSimulatedCredentials(
  address: string | undefined,
  creds: Credential[],
) {
  if (typeof window === "undefined" || !address) return;
...
export default function DashboardPage() {
  ...
  const [selectedForProof, setSelectedForProof] = useState<Credential | null>(
    null,
  );
  ...
  const proofsShared = activityEvents.filter(
    (e) => e.type === "proof_shared",
  ).length;
  ...
  async function handleRevoke(credential: Credential) {
    ...
    const isSimulated = simulatedCredentials.some(
      (c) => c.id === credential.id,
    );

    if (isSimulated) {
      const updated = simulatedCredentials.filter(
        (c) => c.id !== credential.id,
      );
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {stats.map((s) => (
            <div
              key={s.label}
              className="glow-card rounded-xl p-6 bg-surface-900"
            >
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-28 rounded-xl bg-surface-900/60 animate-pulse"
                />
              ))}
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-2 italic">
                    Tip: End with 'a' for Age, 'c' for Citizenship, or 'd' for
                    Degree.
                  </p>
                </div>
